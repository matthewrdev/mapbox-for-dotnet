...
public static class ArtifactScanner
{
        ...
        Action<string> log)
    {
        ...
        {
            nonExistingFixedVersions = LoadFixedVersions(fixedVersionJsonPath);
        }
        ...
        ref List<ArtifactModel> artifacts)
    {
        ...
        if (existingArtifact != null)
        {
            ...
            if (artifactVersion != existingArtifact.Version.SemanticVersion)
            {
                ...
                {
                    var refreshedArtifact = Util.FromArtifactString(
                        basePath,
                    ...
                    existingArtifact.Version = refreshedArtifact.Version;
                    existingArtifact.FixedVersions = LoadFixedVersions(
                        Path.Combine(
                            basePath,
                            existingArtifact.RelativeFolderPath,
                            $"{artifactVersion.ToNormalizedString()}.fixed.json"
                        )
                    ).ToArray();
                }
            }
    ...
    private static List<KeyValuePair<string, string>> LoadFixedVersions(string fixedVersionJsonPath)
    {
        if (!File.Exists(fixedVersionJsonPath))
        {
            return [];
    ...
    {
        var packageSegments = groupId.Split('.')
            .Select(x => x.ToCSharpName())
            .ToArray();

        if (groupId.StartsWith("com.mapbox", StringComparison.Ordinal))
        {
            packageSegments[0] = "Bindings";
        }
